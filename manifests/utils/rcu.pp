...
define orawls::utils::rcu(
  ...
  Enum['adf','soa','mft', 'wcs', 'forms','odi'] $fmw_product = 'adf',
  ...
  if $fmw_product == 'adf' {
    ...
    $components = "-component STB ${rcu_temp_tablespace_cmd} ${rcu_tablespace_cmd} -component ODI  ${rcu_temp_tablespace_cmd} ${rcu_tablespace_cmd} -component IAU ${rcu_temp_tablespace_cmd} ${rcu_tablespace_cmd} -component IAU_VIEWER ${rcu_temp_tablespace_cmd} ${rcu_tablespace_cmd}-component OPSS ${rcu_temp_tablespace_cmd} ${rcu_tablespace_cmd}"
    ...
    $componentsPasswords = [$rcu_password, $rcu_password, $rcu_password,  $rcu_password, $rcu_password,$rcu_password,$rcu_password,'D',"${rcu_prefix}_ODI_REPO",$rcu_password,'AES-128']
  }
  else {
